...
To address these requirements, we have developed a notebook environment that allows modeling studies to be exported using community standards. This workflow %, shown in Fig \ref{fig:workflow},
...
\hline %\thickhline
  ...
  Scanning the steady state of a model as a function of a parameter (\texttt{repeated\_stochastic\_traces.omex}) \\ \hline
  Representing a generalized, time--varying forcing function in SED--ML (\texttt{pulse\_experiment.omex}) \\ \hline
  Generalized, time--varying forcing function in SED--ML (\texttt{pulse\_experiment.omex}) \\ \hline
  Timecourse plot of different parameterizations of a model (\texttt{timecourse\_scan.omex}) \\ \hline
  Nested steady state scan, iterating over two parameters (\texttt{nested\_scan.omex}) \\ \hline
\end{tabular}
