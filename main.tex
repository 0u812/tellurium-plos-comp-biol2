...
\begin{flushleft}
{\Large
\textbf\newline{Reproducibility of Systems Biology Models via Integration of COMBINE
Standards: A Case Study and Implementation} % Please use "sentence case" for title and headings (capitalize only the first word in a title (or heading), the first word in a subtitle (or subheading), and any proper nouns).
}
\newline
...
%\subsection*{Need for Reproducibility in Dynamical Modeling}

% --------------------------------------------------
% FIRST DRAFT OF NEW INTRODUCTION Sept 11
% --------------------------------------------------
...
% For large scale simulations that involve the use of supercomputers and extremely large datasets, it may simply be impractical to attempt to reproduce the results.
...
% * <joseph.hellerstein@gmail.com> 2017-09-12T20:20:41.650Z:
%
% I'd use the term "computational experiment" here since we want to encompass both the model and the simulations.
%
% ^.
\begin{enumerate}
...
%This approach facilitates both transparency, as a result of the notebook narrative, and exchangeability, as a result of Tellurium's fluid standards support.
...
% To facilitate (3), it is convenient to package related standards together. For example, SED--ML specifies how to simulate an SBML or CellML model, and must always be accompanied by the corresponding SBML or CellML file. The COMBINE archive \cite{bergmann2014combine} was created for this purpose, and helps aid reproducibility by preventing mismatches between different versions of models and simulations. We recommend all software which supports SED--ML also support COMBINE archives, which is made easier by a library implementation with multiple language bindings \cite{bergmann_frank_t_2016_154158}.
...
% To demonstrate the Tellurium's capabilities for directly editing standard--encoded models and simulations, and the value these capabilities provide for reusing, validating, and expanding upon previous studies, we constructed a series of case studies. These case studies can be thought of as a suggested workflow for using Tellurium, and also demonstrate the value that COMBINE archives provide for validating the dynamical behavior of models. The first study shows how to convert an existing SBML model, with no accompanying simulation component, into a COMBINE archive containing both SBML and SED--ML. The second study uses a COMBINE archive as a starting point and performs extended validation. Both studies make use of model variants, which are easy to encode in Tellurium. Finally, we present a series of COMBINE archive test cases, which can be used to design and test COMBINE archive support in tools. In all cases, we show that these examples can be exchanged between Tellurium and other tools.
...
% In the previous case study, we examined alternate parameterizations of a model in order to understand how these parameterizations influenced the model's dynamics. The theme of this series of case studies is to show that Tellurium makes it easy to perform additional validation on the behavior of a model. This is in line with our definition of reproducibility, which goes beyond simply replicating previous results. Before using a model, it is important to ensure that the model meets our expectations, including variations on the model. In some cases, as in the previous example, we may not have specific expectations, but we may wish to verify whether a certain model feature is important for the model's behavior. In the previous study, we found that the value chosen for the cooperativity constant was critical.
...
% First, we will study the effect of the translocation rate of MPF into the nuclear volume (Fig \ref{fig:calzone-tr}. As the mitotic divisions are governed by the availability of MPF in the vicinity of the mitotic spindle, lowering the translocation rate essentially slows down the overall dynamics, but does not affect the number of divisions.

% Next, we will study the effect of the nuclear volume fraction (Fig \ref{fig:calzone-e}. This parameter essentially specifies the fraction of the volume of the cell which represents the nuclear ``compartment.'' Having a smaller fraction means that even after several divisions, the nucleus will still only occupy a small fraction of the total cellular volume. A smaller nucleus can continue dividing without depleting the cell's supply of CycB, whereas a larger nucleus depletes the CycB more rapidly an cannot sustain as many divisions. Therefore, the nuclear volume fraction is essential for determining the ``magic'' number of 13 divisions.
...
% % -----------------------------------------------------------------------
%
% Over the past few years, continued progress in dynamical modeling has led researchers to build larger, more comprehensive, and more complex models of biological systems such as the \textit{Mycoplasma genitalium} whole--cell model \cite{karr2012whole} and the central metabolism of \textit{E. coli} \cite{millard2017metabolic}. Large-scale modeling attempts like these are often possible through integration and extension of smaller models that have been proposed and analyzed before.
% In turn, the ability to build on existing submodels depends largely on the accuracy and extensibility of the models, which can be established through several cycles of validation {\color{red} what does validation mean here?} and refinement. Validating models by checking them against multiple sources of independently gathered data {\color{red} According to this definition, validation is about checking a model against experimental data, I agree with this but are you using the word validation with this definition in all cases were you use the word?} is important for continual improvement and refinement of the models. Most models {\color{red} All models!} in biology are incomplete \cite{gunawardena2014models} and must evolve as new data becomes available. For many models, this is difficult to achieve, since the models cannot be easily manipulated {\color{red} Don't agree with this statement}. Finally, as scientific knowledge of a particular system increases, new hypotheses are generated and pre-existing models must be tested against these hypotheses if they are to remain useful. Establishing accurate, trustworthy models through continuous validation {\color{red} Validation against experimental data?} and refinement is an important precursor to the widespread use and adoption of dynamical modeling, as the conclusions drawn from these models can impact future research developments and clinical studies. Without an easy way to modify and test models, validating them against new data or using them for testing new hypotheses becomes impractical.
% % need variations of existing study
% % Put Wolf study here
%
% % Need to emphasize separation btw model & implementation
% % what we can do today is repeatability
% % problem: things are too low-level, solver, mem mgtmt etc.
% % need for another level above implementation: standards, talk about req for standards, then introduce standards
% % then desc of standards, what they do
% % but standards are really detailed
% However, building upon previous work is often difficult because 1) models are often encoded in proprietary or special-purpose software instead of exchangeable standards, and 2) even in the case of open-source modeling tools {\color{red} like what? Copasi, VCell, etc, if so I don't believe you. What do you actually mean here?}, it can be difficult to recreate the environment used to simulate the original model. These issues unnecessarily complicate the process of integration, since researchers might have to reimagine the original model that complies with their data structures, and becomes especially onerous when building large--scale models.
%
% In recognition of this problem, \textbf{reproducibility} has become a central focus of scientific software \cite{peng2011reproducible,sandve2013ten}. However, many software packages which attempt to address the problem of reproducibility actually address what we term \textit{repeatability}, which refers to the ability to recreate a previously derived result using the same software and input data. While this is a useful ability, it does not address challenges such as future--proofing, where a researcher might want to recreate a result decades into the future running on different hardware and a different operating system, or other challenges such as incorporating a previously published model into a larger model. For example, many {\color{red} I suppose this is true.} models are distributed as source code containing a realization of the model as a set of numerical simulation routines, and while this does facilitate repeatability by allowing the same results to be generated again, it does not solve the concerns of reuse, validation, and new hypothesis testing. In general, reproducibility is a gradated term. In computational studies, reproducibility can be divided into the following categories:
%
% \begin{itemize}
% \item \textbf{Computational repeatability:} A result can be replicated with the same model, data, and software.
% \item \textbf{Algorithmic reproducibility:} A result can be replicated with the same model and data but different software which implements the same algorithm.
% \item \textbf{Scientific reproducibility:} A result can be replicated with the same model and data but a different algorithm.
% \item \textbf{Empirical reproducibility:} A result can be replicated with independent model, data, and algorithms.
% \end{itemize}
%
% Repeatability therefore represents the most narrow type of reproducibility. Repeatability tests whether sufficient information exists to recreate a given result {\color{red}  Does it test that? I'm not sure what it tests}. However, it does not test the scientific validity {\color{red}  What does scientific validity mean?} of the result so obtained. Therefore, in this paper, we will adopt a more general definition of reproducibility, aimed at providing at least scientific reproducibility, as defined above.
% %Therefore, in the most stringent sense, reproducibility simply refers to recreating the same result using the same data and software, whereas in a more general and more useful sense, it refers to whether different researchers can converge on the same answer using different algorithms and software. The ability to independently converge on a result is important in scientific research, so in this paper we will adopt the more general definition.
%
% To formalize this notion, we require that our definition of reproducibility incorporate the notions of \textit{transparency} and \textit{exchangeability}.
% \textbf{Transparency} refers to the ability to share procedures or methods in such a way that the individual steps, assumptions, and data sources are clear. Transparency enables other researchers to understand the rationale of a procedure. The critical requirement of transparency is not merely having access to all original software, source code, data, and procedures used to generate a previous result, but also making explicit all assumptions and rationale used to design a model or analysis and conveying this thought process in a lucid way. For example, with transparent model building, it should be possible to answer questions such as ``Where did these parameter values come from?,'' ``Why does this part of the model use lumped kinetics?,'' or ``What was the reason for including only certain interactions?'' without the help of the original authors.
% % transparency isn't something that the software can impose, but it can make it easier or harder
%
% \textbf{Exchangeability} refers to the ability to share data, models, and software with other researchers. Data, models, and software which are encoded in open, standardized formats can be more easily used by other researchers than e.g. software written in a proprietary programming language {\color{red} But most languages are well defined just like SBML and carry no licensing baggage, eg C or Python, what is the argument for using SBML when we could use a standardized language such as C or C++?}, or dynamical models which are tied to a specific software implementation.
% In contrast to transparency, which allows models and simulations to be read and understood by human beings, exchangeability refers to the ability to exchange models and simulations between different software implementations. Many models and simulations are distributed as source code. However, this alone does not guarantee exchangeability if the source code is tied to a particular implementation. In order to achieve exchangeability, it is necessary to separate the \textit{specification} of a model and simulation from its \textit{implementation}. There are a number of well developed open, community--developed standards that can be used to encode specifications, including the Systems Biology Markup Language (SBML) \cite{hucka2003systems,bornstein2008libsbml} and CellML~\cite{cuellar2003overview}, which encode dynamical models, the Simulation Experiment Description Markup Language (SED-ML) \cite{waltemath2011reproducible}, which encodes simulations, NeuroML \cite{gleeson2010neuroml} which encodes data--driven neural models, and the Synthetic Biology Open Language (SBOL) \cite{bartley2015synthetic,galdzicki2014synthetic}, which encodes sequence and functional information of genetic circuits. In turn, various software systems exist which provide an implementation of these standards. For example, a variety of SBML simulators (see \cite{sauro2008standards} % alves2006tools
% for a review) exist which perform the low--level tasks associated with loading and simulating an SBML model, such as constructing and solving differential equations from the model specification, locating steady states, and plotting simulations. Furthermore, exchangeability helps to not only to future--proof models and simulations, but also to identify idiosyncrasies in particular software implementations by comparing results \cite{bergmann2008comparing}.
%
%
% %For example, a wealth of tools support the SBML standard (see \cite{alves2006tools} for a review). A given SBML model should produce convergent results across different tools. This fact has been used to compare the outputs of different SBML simulation tools \cite{bergmann2008comparing}, indicating that more than a third of simulators produce divergent results. This highlights the need for separating the \textit{specification} of a model from its \textit{implementation}. In the example above, SBML is the specification of the model and its implementation is any of the various SBML--compatible software tools available.
% % This is true despite the availability of open community standards for encoding models and simulations such as the Systems Biology Markup Language (SBML) \cite{hucka2003systems,bornstein2008libsbml}, the Simulation Experiment Description Markup Language (SED-ML) \cite{waltemath2011reproducible}, and the COMBINE archive \cite{bergmann2014combine}, which were designed to alleviate these issues by providing a set framework to represent the various parts of a simulation experiment.
%
% % Put in comparison w/ other software
% %The field of systems biology has several academically-developed dynamical modeling environments such as COPASI \cite{hoops2006copasi,mendes2009computational}, CellDesigner \cite{Funahashi2003159, Funahashi2008}, VCell \cite{moraru2008virtual}, PySCeS \cite{olivier2005modelling}, iBioSim \cite{myers2009ibiosim}, PySB \cite{lopez2013programming}, Kappa \cite{feret2009internal}, and RuleBender \cite{smith2012rulebender}. Some of these tools, such as COPASI and iBioSim, offer graphical interfaces for building models. Others, such as PySCeS and PySB, feature a programming API for this purpose. Some of these tools describe dynamical systems, e.g. using ordinary differential equations (ODE) or stochastic master equations, whereas others are designed for simulating rule--based models, which describe interactions between components instead of describing ODEs directly. However, many of these tools are designed for specific problems and/or specific systems, offering limited flexibility. General-purpose simulation tools do exist, such as COPASI which has good coverage of the SBML standard and can be used to simulate many models in the BioModels repository \cite{le2006biomodels,li2010biomodels}. Nevertheless, it is difficult to use software such as COPASI as a single medium for modeling because it does not have general-purpose computing functionality comparable to MATLAB \cite{MATLAB:2016} or Python. Furthermore, even the most widely used software systems in dynamical modeling have limited coverage for newer standards for model exchangeability such as SED-ML and COMBINE archives, necessitating manual intervention when reproducing models.
%
%
%
% %As discussed above, the same model may be implemented in many different ways. This is the reason we have chosen to define reproducibility in terms of the ability to run a given model in a variety of software systems, which we believe is a stronger criterion than merely rerunning a specific implementation of the model. For the same reason, it is also necessary to use open, standard formats for encoding simulation data. Currently, many models are encoded as SBML only, without a simulation component such as SED--ML. This is problematic because it requires manual user intervention in order to faithfully reproduce the model's dynamics, meaning that model repositories such as BioModels \cite{le2006biomodels} cannot automatically be mined for dynamical information. Fig \ref{fig:mistakes} shows some common mistakes in attempting to simulate an SBML model. Users can easily end up using incorrect values for the timescale or simulation algorithm parameters such as tolerances, leading to several cycles of tuning the simulation setup. Also, SBML does not specify whether a model should be simulated using a deterministic or stochastic solver, leading to further ambiguity.
%
% In order to be reproducible, therefore, a model or simulation must be encoded in a standard format which separates its specification from its implementation. However, the standards described above are not human--readable, and therefore an authoring tool is required which allows users to edit the model. This authoring tool should satisfy five requirements:
% \begin{enumerate}
% \item It should represent the models or simulation specifications in a human--readable form
% \item It should allow the user to easily edit this human--readable representation
% \item It should allow the user to provide narrative, annotations, or comments in order to improve transparency {\color{red} Does tellurium notebook allow you to do this, if so can the information be carried over to the modeling standards., or is this a notebook/python special capability?}
...
% * <joseph.hellerstein@gmail.com> 2017-08-04T17:48:48.525Z:
%
% ^.