...
\begin{flushleft}
{\Large
\textbf\newline{Tellurium Notebooks - An Environment for Dynamical Model Development, Reproducibility, and Reuse
% * <joseph.hellerstein@gmail.com> 2017-10-04T16:26:43.237Z:
%
% "Computational Model" seems less descriptive than necessary. Also, what happened to the focus on notebooks? How about
% "Tellurium Notebooks - An Environment for Kinetics Model Development, Reproducibility, and Reuse".
%
% ^.
...
%
% ^ <joseph.hellerstein@gmail.com> 2017-10-04T16:32:59.870Z.
\section*{Abstract}
The considerable difficulty encountered in reproducing the results of published dynamical models limits validation, exploration and reuse of this increasingly large biomedical research resource.   % a problem that is compounded by the fact that there are a large number of published models. SCS--THIS DOESN'T Quite MAKE SENSE TO ME--A LARGE NUMBER OF MODELS DOES NOT COMPOUND THE PROBLEM OF REPRODCIBILITY.
%Lack of reproducibility hinders scientific progress by calling results into question and preventing the reuse of published models.
...
% * <joseph.hellerstein@gmail.com> 2017-10-05T15:41:32.825Z:
%
% What happened to the list sentence in the Author summary?
%
% ^.
...
An authoring tool for reproducibile modeling requires the integration of many different technologies. The steps required to reproduce a simulation from a standard-encoded file comprise a workflow, starting with the import of the file using a standard--supporting library. Next, the imported file must be converted into a human--readable abstraction. Tellurium features notebook cells allow users to this human--readable representation of SBML and SED--ML. If desired, the user can make changes to the human--readable representation before it is automatically re--converted into a standard format and used to perform a simulation. Each of these tasks is performed by a different software package included in Tellurium, as shown in Table \ref{req-implementation}.

% \begin{itemize}
% \item \textbf{libSBML and libSEDML} \cite{hucka2003systems,waltemath2011reproducible} are used to import standard-encoded files.
% \item \textbf{Antimony and PhraSEDML} \cite{smith2009antimony,choi2016phrased} are used to convert these standard encodings to a human--readable representation.
% \item \textbf{libRoadRunner} \cite{somogyi2015libroadrunner} is used to perform simulations.
% \end{itemize}
...
\centering
\caption{
{\bf Requirements and Implementation.} }
\begin{tabular}{p{5cm}|p{5cm}}
\hline %\thickhline
  {\bf Requirement} & {\bf Implementation} \\ \hline
  Import of Standards & libSBML and libSEDML \\ \hline
  Human--readable Specification of Standards & Antimony and PhraSEDML \\ \hline
  Editable Specifications & nteract \\ \hline
\end{tabular}
\begin{flushleft} Integration is a major design goal of Tellurium. We show the different requirements necessary to achieve reproducibility, and the respective software libraries that provide these capabilities.
\end{flushleft}
\label{req-implementation}
%\end{adjustwidth}
\end{table}

While these software libraries can, in principle, be used independently from Tellurium, they do not carry the same benefits for reproducibility when used in isolation. Without integration, a user would need to be familiar with the application programming interface (API) of each of the libraries in the workflow above.

Tellurium's design is defined by three major features: providing software libraries necessary for supporting reproducibility, integration of the steps in the workflow such that the steps can be performed automatically, and providing an interface for authoring models and visualizing the output of simulations. Table \ref{req-implementation} shows the respective software libraries that are used to perform these tasks.

Moreover, Tellurium's notebook interface, based on the \textit{nteract} app \cite{nteract}, allows authoring and editing of human--readable standards in a human--readable representation and provides visualization capabilities for the resulting simulations using Plotly \cite{sievert2016plotly}.
