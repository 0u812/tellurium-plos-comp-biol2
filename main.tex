...
An authoring tool for reproducibile research requires the integration of many different technologies. The steps required to reproduce a simulation from a standard-encoded file comprise a pipeline, starting with the import of the file using a standard--supporting library. Next, the imported file must be converted into some human--readable abstraction. In Tellurium, this abstraction is a notebook cell containing a human--readable representation of SBML and SED--ML. If desired, the user can make changes to this abstraction before it is automatically re--converted into a standard format and used to perform a simulation. In Tellurium, each of these tasks is performed by a different software package:
...
While these software libraries can, in principle, be used independently from Tellurium, they do not carry the same benefits for reproducibility when used in isolation. Without integration, a user would need to be familiar with the application programming interface (API) of each of the libraries in the pipeline above.

Tellurium's design is defined by three major features: providing software libraries necessary for supporting reproducibility, integration of the steps in the pipeline such that the steps can be performed automatically, and providing an interface for authoring models and visualizing the output of simulations.

Moreover, Tellurium's notebook interface, based on the nteract app \cite{nteract} allows authoring and editing of human--readable standards in a human--readable representation and provides visualization capabilities for the resulting simulations using Plotly \cite{sievert2016plotly}.
}
